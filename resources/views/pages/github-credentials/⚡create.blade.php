...
new #[Title('Add GitHub credential')] class extends Component {
    public string $name = '';
    ...
    public function save(): void
    {
        ...
        GithubCredential::create($validated);

        $this->redirect(route('github-credentials.index'), navigate: true);
    }
}; ?>

<div class="max-w-2xl">
    <x-ui.heading size="xl" level="1">{{ __('Add GitHub credential') }}</x-ui.heading>
    <x-ui.subheading class="mb-6">{{ __('An SSH deploy key with read-only access is recommended.') }}</x-ui.subheading>

    <form wire:submit="save" class="space-y-6">
        <x-ui.input wire:model="name" name="name" :label="__('Name')" type="text" required autofocus />

        <x-ui.textarea wire:model="private_key" name="private_key" :label="__('Private key (PEM)')" placeholder="-----BEGIN OPENSSH PRIVATE KEY-----" required />
        ...
        <div class="flex items-center gap-3">
            <flux:button variant="primary" type="submit">{{ __('Create credential') }}</flux:button>
            <flux:button type="button" wire:click="testConnection">{{ __('Test connection') }}</flux:button>
            <flux:button variant="ghost" href="{{ route('github-credentials.index') }}" wire:navigate>{{ __('Cancel') }}</flux:button>
        </div>
    </form>
</div>
